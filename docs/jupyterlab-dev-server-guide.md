...
## 6.1 `docker-compose.dev.proxy.yml` 역할
...
services:
  backend:
    image: biz-ad-ai-backend-dev-spai0913:latest
  ...
  frontend:
    image: biz-ad-ai-frontend-dev-spai0913:latest
...
## 6.3 Docker에서 `API_BASE_URL=http://backend:8010`을 쓰는 이유
...
docker compose \
  ...
  -f docker-compose.dev.proxy.yml \
  -p biz-ad-ai-dev-spai0913 \
  config | grep -E "image:|container_name:|published:"
```
...
```bash
docker exec biz-ad-ai-backend-dev-spai0913 sh -lc 'env | grep -E "KAKAO|FRONTEND|CORS"'
...
8. 개발용 image/container 이름은 배포용과 분리한다.
9. Docker 개발 실행 시 frontend의 API_BASE_URL은 http://backend:8010을 사용한다.
10. API_BROWSER_BASE_URL은 브라우저가 접근 가능한 Jupyter proxy backend 주소를 사용한다.
```
